...
namespace NiobiumStudios
{
    ...
    public class DailyRewardsEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
			...
			SerializedProperty currentRewardSet = serializedObject.FindProperty("CurrentRewardSet");
			SerializedProperty rewardList3 = serializedObject.FindProperty("RewardSets");
			...
			if (EditorTools.DrawHeader("CurrentDailyReward"))
			{
				if (currentRewardSet == null)
				{
					EditorGUILayout.HelpBox("dailyRewards field not found. Check field name in the main script.", MessageType.Error);
					return;
				}

				for (int i = 0; i < currentRewardSet.arraySize; i++)
				{
					SerializedProperty dayProp = currentRewardSet.GetArrayElementAtIndex(i);

					if (EditorTools.DrawHeader("Day " + (i + 1)))
					{
						EditorTools.BeginContents();

						SerializedProperty rewardsListProp = dayProp.FindPropertyRelative("items");

						// List of rewards inside this day
						for (int r = 0; r < rewardsListProp.arraySize; r++)
						{
							SerializedProperty rewardProp = rewardsListProp.GetArrayElementAtIndex(r);

							EditorGUILayout.LabelField("Reward " + (r + 1), EditorStyles.boldLabel);

							var unitProp = rewardProp.FindPropertyRelative("type");
							var rewardQtProp = rewardProp.FindPropertyRelative("quantity");
							var spriteProp = rewardProp.FindPropertyRelative("icon");
							...
							if (GUILayout.Button("Remove Reward"))
							{
								rewardsListProp.DeleteArrayElementAtIndex(r);
							}
						...
						if (GUILayout.Button("Add Reward"))
						{
							rewardsListProp.InsertArrayElementAtIndex(rewardsListProp.arraySize);
						}
						...
						{
							currentRewardSet.DeleteArrayElementAtIndex(i);
						}
					}
				}
				...
				{
					currentRewardSet.InsertArrayElementAtIndex(currentRewardSet.arraySize);
				}
			}

			// This shows the list normally in inspector
			EditorGUILayout.PropertyField(rewardList3, true);
